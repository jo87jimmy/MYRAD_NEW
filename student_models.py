...
class StudentReconstructiveSubNetwork(nn.Module):
    ...
    def forward(self, x, teacher_feat=None, return_features=False):
        """
        x: input image
        teacher_feat: 教師模型瓶頸特徵，用於對齊尺寸
        return_features: 是否返回重建圖 + 投影後瓶頸特徵
        """
        ...
        b_proj = self.bottleneck_proj(b)

        # 如果提供教師特徵，對齊尺寸
        if teacher_feat is not None:
            b_proj = F.interpolate(b_proj, size=(teacher_feat.size(2), teacher_feat.size(3)),
